...
export default function RegisterPage() {
  ...
  const handleGoogleRegister = async () => {
    ...
    try {
      // Simulasi Google OAuth - dalam implementasi nyata akan menggunakan Google OAuth SDK
      // Untuk demo, kita gunakan data dummy
      const googleData = {
        email: 'newuser@gmail.com', // Akan diambil dari Google OAuth
        name: 'New Google User', // Akan diambil dari Google OAuth
        googleId: 'google_' + Date.now() // Akan diambil dari Google OAuth
      };
      ...
      setError(err.response?.data?.error || err.message || 'Registrasi Google gagal');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-white to-gray-200 dark:from-dark-900 dark:to-dark-800">
      <div className="glass-card p-8 rounded-xl w-full max-w-md shadow-lg bg-white text-gray-900 dark:bg-dark-900 dark:text-white">
        <h1 className="text-3xl font-bold text-gradient mb-6 text-center dark:text-blue-700">Daftar</h1>
        ...
        <div className="mt-6 text-center text-gray-600 dark:text-gray-400">
          Sudah punya akun?{' '}
          <Link href="/login" className="text-neon-blue font-semibold hover:underline dark:text-blue-400">Login</Link>
        </div>
      </div>
    </div>
  );
