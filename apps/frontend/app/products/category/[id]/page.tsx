...
export default function CategoryProductsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (minDiscount) filtered = filtered.filter((p) => (p.discount || 0) >= Number(minDiscount));
      if (minStock) filtered = filtered.filter((p) => (p.stock || 0) >= Number(minStock));
      if (isNew) filtered = filtered.filter((p) => p.isNew === (isNew === 'true'));
  ...
  return (
    <div className="min-h-screen pt-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="mb-8">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <aside className="md:w-64 w-full md:sticky md:top-20 flex-shrink-0 z-10">
          <div className="glass-card p-4 space-y-4">
            <select value={params.id} disabled className="glass px-4 py-2 rounded w-full">
              <option value="">{category ? category.name : 'Kategori'}</option>
            </select>
            <input type="number" placeholder="Harga min" value={minPrice} onChange={e => setMinPrice(e.target.value)} className="glass px-4 py-2 rounded w-full" />
            <input type="number" placeholder="Harga max" value={maxPrice} onChange={e => setMaxPrice(e.target.value)} className="glass px-4 py-2 rounded w-full" />
            <input type="number" placeholder="Diskon min %" value={minDiscount} onChange={e => setMinDiscount(e.target.value)} className="glass px-4 py-2 rounded w-full" />
            <input type="number" placeholder="Stok min" value={minStock} onChange={e => setMinStock(e.target.value)} className="glass px-4 py-2 rounded w-full" />
            <select value={isNew} onChange={e => setIsNew(e.target.value)} className="glass px-4 py-2 rounded w-full">
              <option value="">Semua Status</option>
              <option value="true">Produk Baru</option>
              <option value="false">Bukan Produk Baru</option>
            </select>
            <input type="number" placeholder="Rating min" value={minRating} onChange={e => setMinRating(e.target.value)} className="glass px-4 py-2 rounded w-full" />
            <select value={sort} onChange={e => setSort(e.target.value)} className="glass px-4 py-2 rounded w-full">
